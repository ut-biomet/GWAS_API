...
#* Get models list
#* @tag Models
#* @param modelId pattern for filtering according to models's id
#* @param createFrom filter for creation time
#* @param createTo filter for creation time
#* @param markerDataId pattern for filtering according to the marker data used for the model
#* @param phenoDataId pattern for filtering according to the phenotypic data used for the model
#* @param trait pattern for filtering according to the phenotypic trait used for the model
#* @param test pattern for filtering according to the test used for the model
#* @param fixed filter for number of Principal Components to include in the model with fixed effect
#* @param tresh.maf pattern for filtering according to minor allele frequency threshhold used for the model
#* @param tresh.callrate pattern for filtering according to call rate threshhold used for the model
#* @param modelRobjectMD5 pattern for filtering according to the model's R object MD5 sum
#* @param modelFileMD5 pattern for filtering according to the model's file MD5 sum
#* @serializer unboxedJSON
#* @get /models
function(res,
         modelId = NULL,
  ...
  out <- list(
    inputParams = list(
      ...
      tresh.maf = tresh.maf,
      tresh.callrate = tresh.callrate,
      modelRobjectMD5 = modelRobjectMD5,
      modelFileMD5 = modelFileMD5
    )
  )

  cat(as.character(Sys.time()), "-",
      "/models: call with parameters parameters:\n")
  cat("\t", paste(names(out$inputParams),
                  out$inputParams,
                  sep = ": ",
                  collapse = "\n\t"),
      "\n", sep = ""
  ...
  out$models <- getModelList(modelId,
                             locationPath,
                             creationTime,
                             markerDataId,
                             phenoDataId,
                             ...
                             modelFileMD5)
  cat(as.character(Sys.time()), "-",
      "/models: Get model list DONE\n")

  ### RESPONSE
  cat(as.character(Sys.time()), "-",
      "/models: Create response ... \n")
  res$status <- 201 # status for good post response
  out$message <- "Filters are not working for now"
  cat(as.character(Sys.time()), "-",
      "/models: Create response DONE\n")
  cat(as.character(Sys.time()), "-",
      "/models: END \n")
  out
}
